...
if __name__ == '__main__':
    ...
    # Save plot
    plt.savefig(f'latefusion_no_leakage_traininghist_{timestamp}.png')
    plt.show()

    # Save model
    torch.save(sporty_late_fusion.state_dict(), f'latefusion_no_leakage_modelstatedict_{timestamp}.pt')
    torch.save(sporty_late_fusion, f'latefusion_no_leakage_model_{timestamp}.pt')

    # Save training history
    with open(f'latefusion_no_leakage_traininghist_{timestamp}.json', 'w') as f:
        json.dump(out, f)
    ...
    plt.axis('off')
    fig.savefig(f"latefusion_no_leakage_prediction_{timestamp}.png", dpi=300, bbox_inches='tight')
    plt.show()
