...
# 2D CNN used for early fusion 
class Sporty2DNet(nn.Module):
    def __init__(self, T=10):
        super(Sporty2DNet, self).__init__()
        self.convolutional = nn.Sequential(
            nn.Conv2d(in_channels=3*T,
                      kernel_size=7,
                      stride=2,
                      padding=3,
                      out_channels=3*T*2), # Out 32x32x60 RF:7
            nn.BatchNorm2d(3*T*2),
            nn.ReLU(),
            
            nn.Conv2d(in_channels=3*T*2,
                      kernel_size=3,
                      stride=2,
                      padding=1,
                      out_channels=2*3*T*2), # Out 16x16x120 RF:11
            nn.BatchNorm2d(2*3*T*2),
            nn.ReLU(),

            nn.Conv2d(in_channels=2*3*T*2,
                      kernel_size=3,
                      stride=1,
                      padding=1,
                      out_channels=2*3*T*2), # Out 16x16x120 RF:19
            nn.BatchNorm2d(2*3*T*2),
            nn.ReLU(),

            nn.MaxPool2d(kernel_size=2,
                         stride=2), # Out: 8*8*120 RF:23

            nn.Conv2d(in_channels=2*3*T*2,
                      kernel_size=3,
                      stride=1,
                      padding=1,
                      out_channels=2*2*3*T*2), # Out 8x8x240 RF:39
            nn.BatchNorm2d(2*2*3*T*2),
            nn.ReLU(),
            
            nn.Conv2d(in_channels=2*2*3*T*2,
                      kernel_size=3,
                      stride=1,
                      padding=1,
                      out_channels=2*2*3*T*2), # Out 8x8x240 RF:55
            nn.BatchNorm2d(2*2*3*T*2),
            nn.ReLU(),
            ...
            nn.Conv2d(in_channels=2*2*3*T*2,
                      ...
                      stride=1,
                      padding=1,
                      out_channels=2*2*3*T*2), # Out 8x8x240 RF:71
            nn.BatchNorm2d(2*2*3*T*2),
            nn.ReLU(),
        )
            
        self.fully_connected = nn.Sequential(
                nn.Linear(8*8*240, 512),
                nn.ReLU(),
                nn.Dropout(p=0.3),
                nn.Linear(512, 512),
                nn.ReLU(),
...
def train_earlyfusion(model, optimizer, train_loader, test_loader, trainset, testset, num_epochs=10):
...
class FrameImageDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir='ucf101/ucf10', split='train', transform=None):
...
class FrameVideoDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir='ucf101/ucf10', split='train', transform=None, stack_frames=True):
...
class EarlyFusionFrameVideoDataset(torch.utils.data.Dataset):
    def __init__(self, root_dir='ucf101/ucf10', split='train', transform=None, stack_frames=True):
        video_glob = os.path.join(root_dir, 'videos', split, '*', '*.avi')
        self.video_paths = sorted(glob(video_glob))
    ...
    def __getitem__(self, idx):
        ...
        if self.stack_frames:
            ...
            frames = frames.view(-1, 64, 64)  # [30, 64, 64]
...
if __name__ == '__main__':
    ...
    """
    batch_size = 8q
    framevideostack_dataset_train = EarlyFusionFrameVideoDataset(root_dir=root_dir, split='train', transform=transform, stack_frames=True)
    framevideostack_dataset_val = EarlyFusionFrameVideoDataset(root_dir=root_dir, split='val', transform=transform, stack_frames=True)
    ...
    sporty_early_fusion = Sporty2DNet()
    optimizer = torch.optim.Adam(sporty_early_fusion.parameters(), lr=0.00001, weight_decay=1e-4)

    # Send model to GPU
    sporty_early_fusion.to(device)
    ...
    # Train and test
    num_epochs = 15
    out = train_earlyfusion(sporty_early_fusion, optimizer,
    ...
    fig.suptitle('Early Fusion with Sporty2DNet')
    ...
    # Save plot
    plt.savefig(f'earlyfusion_traininghist_{timestamp}.png')
    plt.show()

    # Save model
    torch.save(sporty_early_fusion.state_dict(), f'earlyfusion_modelstatedict_{timestamp}.pt')
    torch.save(sporty_early_fusion, f'earlyfusion_model_{timestamp}.pt')

    # Save training history
    with open(f'earlyfusion_traininghist_{timestamp}.json', 'w') as f:
        json.dump(out, f)
